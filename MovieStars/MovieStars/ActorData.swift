...
// Global array of Actor structs
var actorStructList = [Actor]()
 ...
 ******************************
 */
public func readActorDataFile() {
   
    let actorDataFilename = "MovieStarsData.json"
   
    // Obtain URL of the Movie data file in document directory on user's device
    let urlOfJsonFileInDocumentDirectory = documentDirectory.appendingPathComponent(actorDataFilename)
    ...
    do {
        ...
        actorStructList = loadFromDocumentDirectory(actorDataFilename)
        print("ActorData is loaded from document directory")
       
    } catch {
        // Movie data file does not exist in the document directory; Load it from the main bundle.
       
        actorStructList = loadFromMainBundle(actorDataFilename)
        print("ActorData is loaded from main bundle")
    }
}
 ...
 *****************************************************
 */
public func writeActorDataFile() {
 
    // Obtain the URL of the JSON file in the document directory on the user's device
    let urlOfJsonFileInDocumentDirectory: URL? = documentDirectory.appendingPathComponent("MovieStarsData.json")
 
    let encoder = JSONEncoder()
    if let encoded = try? encoder.encode(actorStructList) {
        do {
            try encoded.write(to: urlOfJsonFileInDocumentDirectory!)
        } catch {
            fatalError("Unable to write encoded Actor data to document directory!")
        }
    } else {
        fatalError("Unable to encodeActor data!")
    }
}
